...
cat_dict={'SLIP':1,'TRIP':2,'CS':3,'HB':4,'RS':5}
def string_to_float(col):
    return float(col)
count=1
for i in range(6,11):   
#     for j in ['SLIP','TRIP']:
    for j in ['SLIP','TRIP','CS','HB','RS']:
        for k in range(1,4):
            train_fall=spark.read.csv('/user/psa83/IMU_Dataset/sub'+\
                                      str(i)+'/Near_Falls/'+j+str(k)+'.csv',header = True)
            subject_id=i
            trial_id=k
            ...
            df_fall.write.csv("NearFalls15.csv", mode="append",header=True,sep=',')
            #if(count==1):
            #    df_final=df_fall
            #else:
