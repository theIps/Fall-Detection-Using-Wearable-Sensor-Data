...
cat_dict={'AS':1,'DS':2,'DSL':3,'DSS':4,'NW':5,'POG':6,'RSS':7,'SQ':8}
def string_to_float(col):
    return float(col)
count=1
for i in range(9,11):   
#     for j in ['SLIP','TRIP']:
    for j in ['AS','DS','DSL','DSS','NW','POG','RSS','SQ']:
        for k in range(1,4):
            train_fall=spark.read.csv('/user/psa83/IMU_Dataset/sub'+\
                                      str(i)+'/ADLs/'+j+str(k)+'.csv',header = True)
            subject_id=i
            trial_id=k
            ...
            df_fall=spark.sql('select * from fall where id between '+str(max_id-60) +' and '+str(max_id+60))
            ...
            df_fall.write.csv("ADLs10.csv", mode="append",header=True,sep=',')
            #if(count==1):
            #    df_final=df_fall
            #else:
            #    df_final=df_final.union(df_fall)
            #count+=1
#df_final.coalesce(1).write.csv('NearFall.csv',sep=',',header=True)
